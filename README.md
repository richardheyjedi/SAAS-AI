...
- **Webhook da MuAPI**: o client (`src/lib/muapi.ts`, função `muApiConfigFromEnv()`) monta a `webhook_url` enviada em cada submissão como `${APP_BASE_URL}/api/webhooks/muapi?secret=${MUAPI_WEBHOOK_SECRET}` (o `?secret=...` só é incluído quando `MUAPI_WEBHOOK_SECRET` está definida). A rota `POST /api/webhooks/muapi` valida esse `?secret=` e rejeita com 401 quem não bater. Ou seja: **defina `MUAPI_WEBHOOK_SECRET` antes de gerar qualquer imagem/vídeo real**, tanto local quanto em produção — sem ela, a URL de webhook fica sem proteção nenhuma (nenhum `?secret=` é anexado) e a MuAPI ainda assim conseguirá postar de volta, mas sem autenticação.
